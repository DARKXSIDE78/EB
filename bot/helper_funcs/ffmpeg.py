...
async def convert_video(video_file, output_directory, total_time, bot, message, chan_msg):
    ...
    audio_b.append("35k")
    file_genertor_command = f"ffmpeg -hide_banner -loglevel quiet -progress '{progress}' -i '{video_file}' -tune animation -metadata 'title=GenAnimeOfc [t.me/GenAnimeOfc]' -metadata 'comment=DARKXSIDE78' -metadata 'creator=DARKXSIDE78' -metadata 'copyright=GenAnimeOfc' -metadata 'album=GenAnimeOfc' -metadata 'genre=Animation' -c:v {codec[0]} -map 0 -crf {crf[0]} -s {resolution[0]} -b:v 150k -c:a libopus -b:a {audio_b[0]} -preset {preset[0]} -metadata:s:v 'title=[GenAnimeOfc]' -metadata:s:a 'title=[GenAnimeOfc]' -metadata:s:s 'title=[GenAnimeOfc]' -metadata 'encoded_by=GenAnimeOfc [DARKXSIDE78]' -metadata 'author=DARKXSIDE78' '{out_put_file_name}' -y"
        ...
        try:
          await message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [ 
                        InlineKeyboardButton('Cᴀɴᴄᴇʟ', callback_data='fuckingdo')
                    ]
                ]
            )
          )
