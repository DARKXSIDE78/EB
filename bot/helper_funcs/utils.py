...
async def sysinfo(e):
    ...
    while time.monotonic() - start_time <= 137:
        ...
        if cpu_freq.min > 0 and cpu_freq.max > 0:
            ...
            freq_info = f"**CPU Fʀᴇǫᴜᴇɴᴄʏ:** {freq_current} (**Mɪɴ:** {freq_min}, **Mᴀx:** {freq_max})\n\n"
        else:
            freq_info = f"**CPU Fʀᴇǫᴜᴇɴᴄʏ:** {freq_current}\n\n"
        ...
        if ram_perc > 80:
            ...
            ram_emoji = "🎮"
        ram_info = f"**RAM Usᴀɢᴇ:** {ram_perc}%\n[{ram_bar}]\n**Usᴇᴅ:** {ram_used} **ᴏғ** {ram_total}\n**Fʀᴇᴇ:**  {psutil._common.bytes2human(ram_stats.available)}\n"
        ...
        disk_info = f"**Dɪsᴋ Usᴀɢᴇ:** {disk_perc}%\n[{disk_bar}]\n**Usᴇᴅ:** {disk_used} **ᴏғ** {disk_total}\n**Fʀᴇᴇ:**  {psutil._common.bytes2human(disk.free)}\n"
        ...
        for i, percent in enumerate(cpu_usage[:cpu_count]):
            if cpu_count > 9 and i < 9:
                ...
                core_num = str(i+1)
            sys_info += f"[{cpu_bar[i]}] **Cᴏʀᴇ {core_num}:** {percent:.1f}%\n"
        sys_info += f"\n\t◉ **Pʜʏsɪᴄᴀʟ Cᴏʀᴇs:** {cpu_count}\n"
        if cpu_count_virtual > 0:
            sys_info += f"\t◉ **Lᴏɢɪᴄᴀʟ Cᴏʀᴇs:** {cpu_count_virtual}\n"
        else:
            sys_info += ""
        sys_info += "".join([""] * 21) + "\n"
        sys_info += ram_info
    ...
    await message.edit_text("**Tɪᴍᴇ Lɪᴍɪᴛ Rᴇᴀᴄʜᴇᴅ!**")
